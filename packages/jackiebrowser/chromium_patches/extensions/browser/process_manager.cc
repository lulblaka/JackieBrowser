...
+  if (jackiebrowser::IsJackieBrowserExtension(worker_id.extension_id)) {
+    base::Uuid keepalive_uuid = IncrementServiceWorkerKeepaliveCount(
+        worker_id,
+        content::ServiceWorkerExternalRequestTimeoutType::kDoesNotTimeout,
+        Activity::PROCESS_MANAGER,
+        "jackiebrowser_permanent_keepalive");
+    jackiebrowser_permanent_keepalives_[worker_id] = keepalive_uuid;
+    VLOG(1) << "jackiebrowser: Added permanent keepalive for extension "
...
+  auto keepalive_iter = jackiebrowser_permanent_keepalives_.find(worker_id);
+  if (keepalive_iter != jackiebrowser_permanent_keepalives_.end()) {
+    DecrementServiceWorkerKeepaliveCount(
+        worker_id, keepalive_iter->second, Activity::PROCESS_MANAGER,
+        "jackiebrowser_permanent_keepalive");
+    jackiebrowser_permanent_keepalives_.erase(keepalive_iter);
+    VLOG(1) << "jackiebrowser: Removed permanent keepalive for extension "
+            << worker_id.extension_id;
+  }
+
