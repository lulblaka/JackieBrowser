...
+
+#if BUILDFLAG(ENABLE_EXTENSIONS)
+  // Transform JackieBrowser extension URLs to display custom names
+  GURL url = location_bar_->GetLocationBarModel()->GetURL();
+  if (url.SchemeIs(extensions::kExtensionScheme)) {
+    std::string extension_id = url.host();
+    if (auto display_name =
+            extensions::jackiebrowser::GetExtensionDisplayName(extension_id)) {
+      std::u16string result = base::UTF8ToUTF16(display_name.value());
+      // Append path if present (e.g., "JackieBrowser-Agent/popup.html")
