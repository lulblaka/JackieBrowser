...
+  
+  // Don't show the dialog for JackieBrowser extensions
+  for (const char* allowed_id : extensions::jackiebrowser::kAllowedExtensions) {
+    if (extension->id() == allowed_id) {
+      LOG(INFO) << "jackiebrowser: Skipping settings override dialog for JackieBrowser extension " 
+                << extension->id();
+      return std::nullopt;
+    }
+  }
