...
+  // Check if it's a JackieBrowser extension
+  if (extensions::jackiebrowser::IsJackieBrowserPinnedExtension(action_id)) {
+    return true;
+  }
+  
   auto* management =
...
+      
+  // Add JackieBrowser extensions that are marked pinned.
+  for (const auto& extension_id : extensions::jackiebrowser::GetJackieBrowserExtensionIds()) {
+    if (!extensions::jackiebrowser::IsJackieBrowserPinnedExtension(extension_id)) {
+      continue;
+    }
