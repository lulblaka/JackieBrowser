...
+#include "chrome/browser/ui/views/side_panel/jackiebrowser_simple_page_extractor.h"
...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "jackiebrowser.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "jackiebrowser.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[jackiebrowser] Provider list size changed from " << previous_size
...
+  
+  jackiebrowser_metrics::JackieBrowserMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[jackiebrowser] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[jackiebrowser] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[jackiebrowser] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[jackiebrowser] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[jackiebrowser] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[jackiebrowser] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[jackiebrowser] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetBrowser().profile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[jackiebrowser] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[jackiebrowser] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  jackiebrowser_metrics::JackieBrowserMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  jackiebrowser_metrics::JackieBrowserMetrics::Log("llmchat.screenshot.captured");
+
...
+  // Use the JackieBrowser simple page extractor
+  std::u16string extracted_text = side_panel::JackieBrowserSimplePageExtractor::ExtractStructuredText(update);
+
+  // Check if we actually got content
+  if (extracted_text.empty()) {
+    LOG(WARNING) << "[jackiebrowser] No content extracted from accessibility tree";
...
+
+  jackiebrowser_metrics::JackieBrowserMetrics::Log("llmchat.content.copied");
+
...
+  }
+
+  LOG(INFO) << "[jackiebrowser] Extracted " << extracted_text.length()
...
+  LOG(INFO) << "[jackiebrowser] Media access requested for origin: "
...
+
+    LOG(INFO) << "[jackiebrowser] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[jackiebrowser] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[jackiebrowser] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
