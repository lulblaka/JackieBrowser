...
@@ -120,6 +122,14 @@ void ExtensionSidePanelManager::MaybeCreateActionItemForExtension(
           ...
           .Build());
+
+  // Auto-pin JackieBrowser extensions to the toolbar.
+  if (jackiebrowser::IsJackieBrowserExtension(extension->id())) {
+    LOG(INFO) << "jackiebrowser: Auto-pinning JackieBrowser extension: " << extension->id();
...
+  
+  // Unpin JackieBrowser extensions before removing the action item
+  if (jackiebrowser::IsJackieBrowserExtension(extension->id())) {
+    LOG(INFO) << "jackiebrowser: Unpinning JackieBrowser extension: " << extension->id() 
