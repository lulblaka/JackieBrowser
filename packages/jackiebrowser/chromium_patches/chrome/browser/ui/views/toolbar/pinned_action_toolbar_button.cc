...
+#include "chrome/browser/ui/actions/jackiebrowser_actions_config.h"
...
+  // Set text from action item if available for JackieBrowser actions
+  if (auto* action_item = container_->GetActionItemFor(action_id)) {
+    if (jackiebrowser::IsJackieBrowserAction(action_id)) {
+      // Check if labels should be shown
+      bool show_labels = true;
+      if (browser_ && browser_->profile()) {
+        show_labels = browser_->profile()->GetPrefs()->GetBoolean(
+            prefs::kJackieBrowserShowToolbarLabels);
+      }
+      else {
+      }
+      
...
+  // Update the text from the action item for JackieBrowser actions
+  if (jackiebrowser::IsJackieBrowserAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
