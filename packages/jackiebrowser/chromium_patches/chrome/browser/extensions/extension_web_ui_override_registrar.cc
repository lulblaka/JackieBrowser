...
+  
+  if (!overrides.empty()) {
+    // Check if this is a JackieBrowser extension
+    bool is_jackiebrowser_extension = false;
+    for (const char* allowed_id : jackiebrowser::kAllowedExtensions) {
+      if (extension->id() == allowed_id) {
+        is_jackiebrowser_extension = true;
+        break;
+      }
+    }
+    
+    if (!is_jackiebrowser_extension) {
