diff --git a/chrome/browser/extensions/jackiebrowser_external_loader.h b/chrome/browser/extensions/jackiebrowser_external_loader.h
new file mode 100644
index 0000000000000..b313a0ba6fa10
--- /dev/null
+++ b/chrome/browser/extensions/jackiebrowser_external_loader.h
@@ -0,0 +1,126 @@
...
+// from a remote URL. This is designed for JackieBrowser to specify a set of
+// extensions that should be installed at startup.
+class JackieBrowserExternalLoader : public ExternalLoader {
+ public:
+  explicit JackieBrowserExternalLoader(Profile* profile);
+
+  JackieBrowserExternalLoader(const JackieBrowserExternalLoader&) = delete;
+  JackieBrowserExternalLoader& operator=(const JackieBrowserExternalLoader&) = delete;
+
...
+  
+  // Forces immediate update check for JackieBrowser extensions
+  void ForceUpdateCheck();
+
+ protected:
+  ~JackieBrowserExternalLoader() override;
+
+  // ExternalLoader:
...
+  
+  // Checks for uninstalled JackieBrowser extensions and reinstalls them
+  void ReinstallUninstalledExtensions();
+  
+  // Re-enables JackieBrowser extensions that were disabled by user action
+  void ReenableDisabledExtensions();
+  
+  // Triggers immediate installation of all JackieBrowser extensions on first start
...
+
+  // List of JackieBrowser extension IDs to monitor
+  std::set<std::string> jackiebrowser_extension_ids_;
+  
...
+
+  base::WeakPtrFactory<JackieBrowserExternalLoader> weak_ptr_factory_{this};
+};
+
+}  // namespace extensions
+
