...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[jackiebrowser] No RenderFrameHost for AccessibilityDoDefault";
+    return false;
+  }
+  
...
+  
+  LOG(INFO) << "[jackiebrowser] Performing AccessibilityDoDefault on node " 
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[jackiebrowser] No RenderFrameHost for AccessibilityFocus";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  action_data.target_tree_id = node_info.ax_tree_id;
+  
+  LOG(INFO) << "[jackiebrowser] Performing AccessibilityFocus on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[jackiebrowser] No RenderFrameHost for AccessibilityScrollToMakeVisible";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[jackiebrowser] Performing AccessibilityScrollToMakeVisible on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+    LOG(INFO) << "[jackiebrowser] Node is out of viewport, scrolling to make visible";
...
+    // LOG(INFO) << "[jackiebrowser] Node was out of viewport, trying AccessibilityDoDefault click first";
+    // bool changed = JackieBrowserChangeDetector::ExecuteWithDetection(
+    //     web_contents,
...
+      LOG(INFO) << "[jackiebrowser] No change from accessibility click, trying HTML click";
+      changed = JackieBrowserChangeDetector::ExecuteWithDetection(
+          web_contents,
+          [&]() { HtmlClick(web_contents, node_info); },
+          base::Milliseconds(200));
+    }
+    
+    LOG(INFO) << "[jackiebrowser] Click result: " << (changed ? "changed" : "no change");
+    return changed;
+  }
+  
+  // For in-viewport nodes, try coordinate click first (most natural)
+  LOG(INFO) << "[jackiebrowser] Node is in viewport, trying coordinate click first";
+  gfx::PointF click_point = GetNodeCenterPoint(web_contents, node_info);
+  
+  bool changed = JackieBrowserChangeDetector::ExecuteWithDetection(
+      web_contents,
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[jackiebrowser] No RenderFrameHost for AccessibilitySetValue";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[jackiebrowser] Performing AccessibilitySetValue on node " 
...
+  
+  if (is_out_of_viewport) {
+    LOG(INFO) << "[jackiebrowser] Node is out of viewport for typing, scrolling to make visible";
+    AccessibilityScrollToMakeVisible(web_contents, node_info, true /* center */);
+    // Wait for scroll to complete
+    base::PlatformThread::Sleep(base::Milliseconds(300));
+  }
...
+  if (!changed) {
+    LOG(INFO) << "[jackiebrowser] No change from native typing, trying JavaScript";
+    changed = JackieBrowserChangeDetector::ExecuteWithDetection(
+        web_contents,
...
+  // if (!changed) {
+  //   LOG(INFO) << "[jackiebrowser] No change from JavaScript, trying accessibility SetValue";
+  //   changed = JackieBrowserChangeDetector::ExecuteWithDetection(
+  //       web_contents,
+  //       [&]() {
...
+  // }
+  
+  LOG(INFO) << "[jackiebrowser] Type result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[jackiebrowser] Clear result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[jackiebrowser] KeyPress result for '" << key << "': " 
...
+    LOG(INFO) << "[jackiebrowser] No interactive elements in viewport to highlight";
+    return;
+  }
+  
+  LOG(INFO) << "[jackiebrowser] Highlighting " << filtered_nodes.size() 
...
+      document.querySelectorAll('.jackiebrowser-bbox').forEach(e => e.remove());
+      
+      // Create container for all bounding boxes
+      const container = document.createElement('div');
+      container.className = 'jackiebrowser-bbox-container';
...
+        box.className = 'jackiebrowser-bbox';
...
+      document.querySelectorAll('.jackiebrowser-bbox-container').forEach(e => e.remove());
+      document.querySelectorAll('.jackiebrowser-bbox').forEach(e => e.remove());
+      
+      // Remove all highlight containers  
+      document.querySelectorAll('.jackiebrowser-highlight-container').forEach(e => e.remove());
+      document.querySelectorAll('.jackiebrowser-highlight').forEach(e => e.remove());
+      
+      // Remove any style elements we added
+      document.querySelectorAll('#jackiebrowser-highlight-styles').forEach(e => e.remove());
+      
+      return true;
+    })();
+  )";
...
+                                   const gfx::PointF& point) {
+  LOG(INFO) << "[jackiebrowser] ClickCoordinatesWithDetection at (" 
+            << point.x() << ", " << point.y() << ")";
+  
...
+      base::Milliseconds(300));
+  
+  LOG(INFO) << "[jackiebrowser] Click coordinates result: " 
+            << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+                                    const gfx::PointF& point,
+                                    const std::string& text) {
+  LOG(INFO) << "[jackiebrowser] TypeAtCoordinatesWithDetection at (" 
...
+  if (!changed) {
+    LOG(INFO) << "[jackiebrowser] No change from native typing at coordinates, trying JS injection";
+    
...
+  }
+  
+  LOG(INFO) << "[jackiebrowser] Type at coordinates result: " 
