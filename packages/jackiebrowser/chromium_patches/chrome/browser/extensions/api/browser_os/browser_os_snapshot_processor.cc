...
+            &is_offscreen);
+        
+        VLOG(3) << "[jackiebrowser] Node " << node_data.id 
...
+    } else {
+      // No AXTree available
+      LOG(WARNING) << "[jackiebrowser] No AXTree available for bounds computation";
+    }
+    
...
+  }
+  
+  LOG(INFO) << "[jackiebrowser] Viewport: " << viewport_size.ToString() 
...
+  
+  if (!ax_tree) {
+    LOG(ERROR) << "[jackiebrowser] Failed to create AXTree from update";
+    SnapshotProcessingResult result;
...
+  }
+  
+  LOG(INFO) << "[jackiebrowser] Created AXTree with " << tree_update.nodes.size() 
