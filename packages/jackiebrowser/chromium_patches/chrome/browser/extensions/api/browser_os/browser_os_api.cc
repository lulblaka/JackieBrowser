...
+uint32_t JackieBrowserGetInteractiveSnapshotFunction::next_snapshot_id_ = 1;
+
+// Constructor and destructor implementations
+JackieBrowserGetInteractiveSnapshotFunction::JackieBrowserGetInteractiveSnapshotFunction() = default;
+JackieBrowserGetInteractiveSnapshotFunction::~JackieBrowserGetInteractiveSnapshotFunction() = default;
+
+ExtensionFunction::ResponseAction JackieBrowserGetAccessibilityTreeFunction::Run() {
...
+      base::BindOnce(
+          &JackieBrowserGetAccessibilityTreeFunction::OnAccessibilityTreeReceived,
+          this),
...
+}
+
+void JackieBrowserGetAccessibilityTreeFunction::OnAccessibilityTreeReceived(
...
+}
+
+// Implementation of JackieBrowserGetInteractiveSnapshotFunction
+
+ExtensionFunction::ResponseAction JackieBrowserGetInteractiveSnapshotFunction::Run() {
...
+    LOG(WARNING) << "[jackiebrowser] Frame not stable for AX snapshot - skipping";
...
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(
+          &JackieBrowserGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived,
+          this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties |
+                 ui::AXMode::kInlineTextBoxes),
+      /* max_nodes= */ 0,  // No limit
...
+}
+
+void JackieBrowserGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  // Double-check frame is still valid before processing
+  if (!web_contents_) {
+    LOG(WARNING) << "[jackiebrowser] WebContents gone during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+    LOG(WARNING) << "[jackiebrowser] Frame became unstable during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+      web_contents_,
+      base::BindOnce(
+          &JackieBrowserGetInteractiveSnapshotFunction::OnSnapshotProcessed,
+          base::WrapRefCounted(this)));
+}
+
+void JackieBrowserGetInteractiveSnapshotFunction::OnSnapshotProcessed(
+    SnapshotProcessingResult result) {
+  Respond(ArgumentList(
+      browser_os::GetInteractiveSnapshot::Results::Create(result.snapshot)));
+}
+
+// Implementation of JackieBrowserClickFunction
+
+ExtensionFunction::ResponseAction JackieBrowserClickFunction::Run() {
...
+}
+
+// Implementation of JackieBrowserInputTextFunction
+
+ExtensionFunction::ResponseAction JackieBrowserInputTextFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[jackiebrowser] InputText: Starting input for nodeId: " << params->node_id;
+  
...
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[jackiebrowser] InputText: No change detected after typing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of JackieBrowserClearFunction
+
+ExtensionFunction::ResponseAction JackieBrowserClearFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[jackiebrowser] Clear: Clearing field for nodeId: " << params->node_id;
+  
+  // Use ClearWithDetection which handles focus and clearing
+  bool change_detected = ClearWithDetection(web_contents, node_info);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[jackiebrowser] Clear: No change detected after clearing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of JackieBrowserGetPageLoadStatusFunction
+
+ExtensionFunction::ResponseAction JackieBrowserGetPageLoadStatusFunction::Run() {
...
+}
+
+// Implementation of JackieBrowserScrollUpFunction
+
+ExtensionFunction::ResponseAction JackieBrowserScrollUpFunction::Run() {
...
+}
+
+// Implementation of JackieBrowserScrollDownFunction
+
+ExtensionFunction::ResponseAction JackieBrowserScrollDownFunction::Run() {
...
+}
+
+// Implementation of JackieBrowserScrollToNodeFunction
+
+ExtensionFunction::ResponseAction JackieBrowserScrollToNodeFunction::Run() {
...
+}
+
+// Implementation of JackieBrowserSendKeysFunction
+
+ExtensionFunction::ResponseAction JackieBrowserSendKeysFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[jackiebrowser] SendKeys: Sending key '" << params->key << "'";
+  
+  // Send the key with change detection
+  bool change_detected = KeyPressWithDetection(web_contents, params->key);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[jackiebrowser] SendKeys: No change detected after key press";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of JackieBrowserCaptureScreenshotFunction
+
+JackieBrowserCaptureScreenshotFunction::JackieBrowserCaptureScreenshotFunction() = default;
+JackieBrowserCaptureScreenshotFunction::~JackieBrowserCaptureScreenshotFunction() = default;
+
+ExtensionFunction::ResponseAction JackieBrowserCaptureScreenshotFunction::Run() {
...
+    LOG(INFO) << "[jackiebrowser] CaptureScreenshot: Using exact dimensions: "
...
+      LOG(INFO) << "[jackiebrowser] CaptureScreenshot: Using thumbnail size: " << max_dimension 
...
+      LOG(INFO) << "[jackiebrowser] CaptureScreenshot: Using viewport size: " << max_dimension;
+    }
+    
...
+}
+
+void JackieBrowserCaptureScreenshotFunction::DrawHighlightsAndCapture() {
...
+      LOG(INFO) << "[jackiebrowser] Drawing highlights for screenshot with " 
+                << tab_it->second.size() << " interactive elements";
+      ShowHighlights(web_contents_, tab_it->second, true /* show_labels */);
+    } else {
+      LOG(INFO) << "[jackiebrowser] No snapshot data available for highlighting";
+    }
+    
...
+        FROM_HERE,
+        base::BindOnce(&JackieBrowserCaptureScreenshotFunction::CaptureScreenshotNow,
+                       base::WrapRefCounted(this)),
...
+}
+
+void JackieBrowserCaptureScreenshotFunction::CaptureScreenshotNow() {
+  if (!web_contents_) {
...
+      target_size_,
+      base::BindOnce(&JackieBrowserCaptureScreenshotFunction::OnScreenshotCaptured,
+                     this));
+}
+
+void JackieBrowserCaptureScreenshotFunction::OnScreenshotCaptured(
...
+}
+
+// JackieBrowserGetSnapshotFunction implementation
+ExtensionFunction::ResponseAction JackieBrowserGetSnapshotFunction::Run() {
...
+  // Request accessibility tree snapshot
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(&JackieBrowserGetSnapshotFunction::OnAccessibilityTreeReceived,
+                     this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties),
+      /* max_nodes= */ 0,  // No limit
+      /* timeout= */ base::TimeDelta(),
...
+}
+
+void JackieBrowserGetSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  if (!has_callback()) {
+    return;
+  }
...
+}
+
+// JackieBrowserGetPrefFunction
+ExtensionFunction::ResponseAction JackieBrowserGetPrefFunction::Run() {
...
+}
+
+// JackieBrowserSetPrefFunction
+ExtensionFunction::ResponseAction JackieBrowserSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying jackiebrowser.* prefs
+  if (!params->name.starts_with("jackiebrowser.")) {
+    return RespondNow(Error("Only jackiebrowser.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+// JackieBrowserGetAllPrefsFunction
+ExtensionFunction::ResponseAction JackieBrowserGetAllPrefsFunction::Run() {
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* profile_prefs = profile->GetPrefs();
+  PrefService* local_state = g_browser_process->local_state();
+
+  // Build a combined jackiebrowser prefs dict from both sources
+  base::Value::Dict combined_jackiebrowser;
+
+  // Lambda to merge jackiebrowser prefs from a PrefService
...
+
+    // Look for "jackiebrowser" key in the top-level dict
+    const base::Value* jackiebrowser_value = pref_dict.Find("jackiebrowser");
+    if (jackiebrowser_value && jackiebrowser_value->is_dict()) {
+      // Merge this jackiebrowser dict into combined
+      combined_jackiebrowser.Merge(jackiebrowser_value->GetDict().Clone());
+      LOG(INFO) << "[jackiebrowser] GetAllPrefs: Found jackiebrowser.* prefs in " << source_name;
+    }
+  };
+
...
+
+  // Create single PrefObject with the entire jackiebrowser dict
+  std::vector<browser_os::PrefObject> pref_objects;
+  browser_os::PrefObject pref_obj;
+  pref_obj.key = "jackiebrowser";
+  pref_obj.type = "dictionary";
+  pref_obj.value = base::Value(std::move(combined_jackiebrowser));
...
+}
+
+// JackieBrowserLogMetricFunction
+ExtensionFunction::ResponseAction JackieBrowserLogMetricFunction::Run() {
...
+    
+    jackiebrowser_metrics::JackieBrowserMetrics::Log(prefixed_event, std::move(properties));
+  } else {
+    // No properties, just log with extension ID
+    jackiebrowser_metrics::JackieBrowserMetrics::Log(prefixed_event, {
...
+}
+
+// JackieBrowserGetVersionNumberFunction
+ExtensionFunction::ResponseAction JackieBrowserGetVersionNumberFunction::Run() {
...
+}
+
+// JackieBrowserExecuteJavaScriptFunction
+ExtensionFunction::ResponseAction JackieBrowserExecuteJavaScriptFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[jackiebrowser] ExecuteJavaScript: Executing code in tab " << tab_info->tab_id;
+  
...
+      js_code,
+      base::BindOnce(&JackieBrowserExecuteJavaScriptFunction::OnJavaScriptExecuted,
+                     this),
...
+}
+
+void JackieBrowserExecuteJavaScriptFunction::OnJavaScriptExecuted(base::Value result) {
+  LOG(INFO) << "[jackiebrowser] ExecuteJavaScript: Execution completed";
+
+  if (result.is_none()) {
...
+}
+
+// Implementation of JackieBrowserClickCoordinatesFunction
+ExtensionFunction::ResponseAction JackieBrowserClickCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[jackiebrowser] ClickCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
...
+  
+  LOG(INFO) << "[jackiebrowser] ClickCoordinates: Clicking at (" 
+            << params->x << ", " << params->y << ")";
+  
...
+  
+  LOG(INFO) << "[jackiebrowser] ClickCoordinates: Result = " 
...
+}
+
+// Implementation of JackieBrowserTypeAtCoordinatesFunction  
+ExtensionFunction::ResponseAction JackieBrowserTypeAtCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[jackiebrowser] TypeAtCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
+    response.success = false;
+    return RespondNow(ArgumentList(
...
+  gfx::PointF click_point(params->x, params->y);
+  
+  LOG(INFO) << "[jackiebrowser] TypeAtCoordinates: Clicking at (" 
...
+  response.success = success;
+  
+  LOG(INFO) << "[jackiebrowser] TypeAtCoordinates: Result = " 
