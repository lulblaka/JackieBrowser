...
+    LOG(INFO) << "jackiebrowser: ExtractPageContent - tree is empty";
+    return items;
+  }
+
+  LOG(INFO) << "jackiebrowser: ExtractPageContent - processing " << tree_update.nodes.size() << " nodes";
+
...
+
+  LOG(INFO) << "jackiebrowser: ExtractPageContent - extracted " << items.size() << " items";
+
+  return items;
+}
+
