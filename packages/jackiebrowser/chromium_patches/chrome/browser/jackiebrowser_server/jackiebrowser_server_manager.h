diff --git a/chrome/browser/jackiebrowser_server/jackiebrowser_server_manager.h b/chrome/browser/jackiebrowser_server/jackiebrowser_server_manager.h
new file mode 100644
index 0000000000000..d991d965c4913
--- /dev/null
+++ b/chrome/browser/jackiebrowser_server/jackiebrowser_server_manager.h
@@ -0,0 +1,138 @@
...
+// 2. Launches the bundled JackieBrowser server binary with CDP and MCP ports
+// 3. Monitors MCP server health via HTTP /health endpoint and auto-restarts
+class JackieBrowserServerManager {
+ public:
+  static JackieBrowserServerManager* GetInstance();
+
+  JackieBrowserServerManager(const JackieBrowserServerManager&) = delete;
+  JackieBrowserServerManager& operator=(const JackieBrowserServerManager&) = delete;
+
+  // Starts the JackieBrowser server if not already running
+  // This will:
...
+  // 4. Launch jackiebrowser_server binary with discovered ports
+  void Start();
+
+  // Stops the JackieBrowser server
+  void Stop();
+
...
+
+ private:
+  friend base::NoDestructor<JackieBrowserServerManager>;
+
+  JackieBrowserServerManager();
+  ~JackieBrowserServerManager();
+
+  bool AcquireLock();
...
+  void StartCDPServer();
+  void StopCDPServer();
+  void LaunchJackieBrowserProcess();
+  void OnProcessLaunched(base::Process process);
+  void TerminateJackieBrowserProcess();
+  void RestartJackieBrowserProcess();
...
+
+  base::FilePath GetJackieBrowserServerResourcesPath() const;
+  base::FilePath GetJackieBrowserExecutionDir() const;
+  base::FilePath GetJackieBrowserServerExecutablePath() const;
...
+
+  base::WeakPtrFactory<JackieBrowserServerManager> weak_factory_{this};
+};
+
+}  // namespace jackiebrowser
+
