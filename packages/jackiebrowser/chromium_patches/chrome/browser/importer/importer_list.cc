...
+  LOG(INFO) << "jackiebrowser: Checking for extensions in: " << preferences_path.AsUTF8Unsafe();
+
+  std::string preferences_content;
+  if (!base::ReadFileToString(preferences_path, &preferences_content)) {
+    LOG(INFO) << "jackiebrowser: Failed to read preferences file: " << preferences_path.AsUTF8Unsafe();
+    return false;
+  }
+
...
+  if (!preferences) {
+    LOG(INFO) << "jackiebrowser: Failed to parse preferences file as JSON: " << preferences_path.AsUTF8Unsafe();
+    return false;
+  }
+
...
+  if (!extensions_dict) {
+    LOG(INFO) << "jackiebrowser: No extensions.settings found in preferences file";
+    return false;
+  }
+
+  LOG(INFO) << "jackiebrowser: Found extensions.settings with " << extensions_dict->size() << " entries";
+
...
+      LOG(INFO) << "jackiebrowser: Extension " << key << " was installed by default, skipping";
+      continue;  // Skip default extensions
+    }
+
+    if (!dict.FindBool("from_webstore").value_or(false)) {
+      LOG(INFO) << "jackiebrowser: Extension " << key << " is not from the web store, skipping";
+      continue;  // Skip non-webstore extensions
+    }
+    return true;
+  }
+
+  LOG(INFO) << "jackiebrowser: Examined " << examined_extensions << " extensions, none qualified for import";
+  return false;
+}
+
