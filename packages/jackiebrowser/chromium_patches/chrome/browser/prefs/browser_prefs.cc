...
+#include "components/metrics/jackiebrowser_metrics/jackiebrowser_metrics_prefs.h"
...
+  jackiebrowser_server::RegisterLocalStatePrefs(registry);
+  jackiebrowser_metrics::RegisterLocalStatePrefs(registry);
...
+  jackiebrowser_metrics::RegisterProfilePrefs(registry);
...
+  RegisterJackieBrowserPrefs(registry);
...
+void RegisterJackieBrowserPrefs(user_prefs::PrefRegistrySyncable* registry) {
...
+  registry->RegisterStringPref(prefs::kJackieBrowserProviders, "");
+  
+  // JackieBrowser toolbar settings
+  registry->RegisterBooleanPref(prefs::kJackieBrowserShowToolbarLabels, true);
+  
+  // Custom providers list - stored as a JSON string
+  registry->RegisterStringPref(prefs::kJackieBrowserCustomProviders, "[]");
+}
+
 #if BUILDFLAG(IS_CHROMEOS)
