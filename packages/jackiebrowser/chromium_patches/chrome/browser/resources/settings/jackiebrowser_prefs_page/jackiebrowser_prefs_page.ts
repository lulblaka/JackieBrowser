diff --git a/chrome/browser/resources/settings/jackiebrowser_prefs_page/jackiebrowser_prefs_page.ts b/chrome/browser/resources/settings/jackiebrowser_prefs_page/jackiebrowser_prefs_page.ts
new file mode 100644
index 0000000000000..2616c77cc001b
--- /dev/null
+++ b/chrome/browser/resources/settings/jackiebrowser_prefs_page/jackiebrowser_prefs_page.ts
@@ -0,0 +1,250 @@
...
+
+/**
+ * @fileoverview 'settings-jackiebrowser-prefs-page' contains JackieBrowser-specific settings.
+ */
+
...
+
+import {getTemplate} from './jackiebrowser_prefs_page.html.js';
+
...
+}
+
+export interface SettingsJackieBrowserPrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const SettingsJackieBrowserPrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class SettingsJackieBrowserPrefsPageElement extends SettingsJackieBrowserPrefsPageElementBase {
+  static get is() {
+    return 'settings-jackiebrowser-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('jackiebrowser.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('jackiebrowser.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+    'settings-jackiebrowser-prefs-page': SettingsJackieBrowserPrefsPageElement;
+  }
+}
+
+customElements.define(
+    SettingsJackieBrowserPrefsPageElement.is, SettingsJackieBrowserPrefsPageElement);
