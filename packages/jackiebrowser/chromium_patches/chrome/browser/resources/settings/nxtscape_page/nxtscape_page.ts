...
+    if (this.prefs && this.prefs.jackiebrowser) {
+      this.loadProviders_();
+    }
+  }
+
+  private loadProviders_() {
+    // Load from preferences or initialize with JackieBrowser
+    if (!this.prefs || !this.prefs.jackiebrowser || !this.prefs.jackiebrowser.providers) {
+      // Prefs not ready yet, initialize defaults locally only
+      this.initializeDefaultProvidersLocally_();
+      return;
+    }
+    
+    const stored = this.getPref('jackiebrowser.providers');
...
+        
+        // Ensure JackieBrowser is always present
+        if (!this.providers_.some(p => p.id === 'jackiebrowser')) {
...
+    
+    const jackiebrowser: ProviderConfig = {
+      id: 'jackiebrowser',
+      name: 'JackieBrowser',
...
+    };
+    
+    this.providers_ = [jackiebrowser];
+    this.defaultProviderId_ = 'jackiebrowser';
...
+    if (this.prefs && this.prefs.jackiebrowser && this.prefs.jackiebrowser.providers !== undefined) {
+      this.saveProviders_();
+    }
+  }
+
...
+    if (!this.prefs || !this.prefs.jackiebrowser || this.prefs.jackiebrowser.providers === undefined) {
+      console.warn('jackiebrowser: Cannot save providers - prefs not ready');
+      return;
+    }
+    
...
+    this.setPrefValue('jackiebrowser.providers', JSON.stringify(data));
+    console.log('jackiebrowser: Saving providers:', data);
+  }
+
...
+    console.log('jackiebrowser: Add Provider clicked');
+    
...
+    }
+    
+    console.log('jackiebrowser: Form visibility:', this.showProviderForm_);
+  }
+
...
+    const provider = this.providers_.find(p => p.id === providerId);
+    
+    console.log('jackiebrowser: Delete provider clicked:', providerId, provider);
+    
...
+      
+      // If deleted provider was default, set JackieBrowser as default
+      if (provider.id === this.defaultProviderId_) {
+        this.set('defaultProviderId_', 'jackiebrowser');
...
+    }
+    
+    console.log('jackiebrowser: Provider type changed to:', providerType);
+    
+    const defaults = PROVIDER_DEFAULTS[providerType];
+    if (defaults) {
+      console.log('jackiebrowser: Applying defaults:', defaults);
...
+            console.log('jackiebrowser: Auto-set context length to:', contextLength);
+          }
+          
+          this.set('showModelDropdown_', false);
+          this.set('selectedSuggestionIndex_', -1);
...
+    console.log('jackiebrowser: Model suggestion clicked:', model);
+    
+    if (model) {
+      console.log('jackiebrowser: Setting dialogModelId_ to:', model);
...
+      if (input) {
+        input.value = model;
+        console.log('jackiebrowser: Input value set to:', input.value);
+      }
+    }
+  }
+  
...
+    console.log('jackiebrowser: Custom model selected:', this.dialogModelId_);
+    
...
+      
+      chrome.send('logJackieBrowserMetric', ['settings.provider.added', {
...
+    console.log('jackiebrowser: Validating form:', {
+      name: this.dialogProviderName_,
+      type: this.dialogProviderType_,
+      apiKey: this.dialogApiKey_,
+      modelId: this.dialogModelId_
...
+
+    chrome.send('logJackieBrowserMetric', ['settings.default_provider.changed', {
...
+    }, 600);
+    
+    console.log('jackiebrowser: Using template:', template.name);
+  }
+
