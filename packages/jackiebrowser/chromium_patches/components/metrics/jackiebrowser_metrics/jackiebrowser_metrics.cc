diff --git a/components/metrics/jackiebrowser_metrics/jackiebrowser_metrics.cc b/components/metrics/jackiebrowser_metrics/jackiebrowser_metrics.cc
new file mode 100644
index 0000000000000..02a1dc121bb7a
--- /dev/null
+++ b/components/metrics/jackiebrowser_metrics/jackiebrowser_metrics.cc
@@ -0,0 +1,100 @@
...
+JackieBrowserMetricsService* GetMetricsService() {
...
+  return JackieBrowserMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "jackiebrowser: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void JackieBrowserMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::Value::Dict(), sample_rate);
+}
+
+// static
+void JackieBrowserMetrics::Log(const std::string& event_name,
...
+
+// static
+void JackieBrowserMetrics::Log(const std::string& event_name, base::Value::Dict properties,
+                           double sample_rate) {
