diff --git a/components/metrics/jackiebrowser_metrics/jackiebrowser_metrics_service.h b/components/metrics/jackiebrowser_metrics/jackiebrowser_metrics_service.h
new file mode 100644
index 0000000000000..cb7519c887826
--- /dev/null
+++ b/components/metrics/jackiebrowser_metrics/jackiebrowser_metrics_service.h
@@ -0,0 +1,95 @@
...
+class JackieBrowserMetricsService : public KeyedService {
+ public:
+  explicit JackieBrowserMetricsService(
+      PrefService* pref_service,
+      PrefService* local_state_prefs,
+      scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
+
+  JackieBrowserMetricsService(const JackieBrowserMetricsService&) = delete;
+  JackieBrowserMetricsService& operator=(const JackieBrowserMetricsService&) = delete;
+
+  ~JackieBrowserMetricsService() override;
+
...
+  base::WeakPtrFactory<JackieBrowserMetricsService> weak_factory_{this};
+};
+
+}  // namespace jackiebrowser_metrics
+
