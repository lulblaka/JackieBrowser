...
BROWSEROS_SERVER_BINARIES: Dict[str, Dict[str, str]] = {
    "jackiebrowser_server": {
        "identifier_suffix": "jackiebrowser_server",
        "options": "runtime",
        "entitlements": "jackiebrowser-executable-entitlements.plist",
    },
    "codex": {
...
def get_jackiebrowser_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
...
def find_components_to_sign(
    ...
    jackiebrowser_server_dir = join_paths(app_path, "Contents", "Resources", "BrowserOSServer")
    if jackiebrowser_server_dir.exists():
        for item in jackiebrowser_server_dir.rglob("*"):
            if item.is_file() and not item.suffix and os.access(item, os.X_OK):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.jackiebrowser"
) -> str:
    ...
    jackiebrowser_server_info = get_jackiebrowser_server_binary_info(component_path)
    if jackiebrowser_server_info:
        suffix = jackiebrowser_server_info.get("identifier_suffix", component_path.stem)
...
def get_signing_options(component_path: Path) -> str:
    ...
    jackiebrowser_server_info = get_jackiebrowser_server_binary_info(component_path)
    if jackiebrowser_server_info:
        return jackiebrowser_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            jackiebrowser_server_info = get_jackiebrowser_server_binary_info(exe)
            if jackiebrowser_server_info:
                entitlements_name = jackiebrowser_server_info.get("entitlements")
    ...
    if not sign_component(main_exe, certificate_name, "com.jackiebrowser.BrowserOS"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.jackiebrowser.BrowserOS" and '
