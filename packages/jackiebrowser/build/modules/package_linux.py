...
def copy_browser_files(
    ...
    jackiebrowser_path = Path(join_paths(target_dir, ctx.NXTSCAPE_APP_NAME))
    if jackiebrowser_path.exists():
        jackiebrowser_path.chmod(0o755)
...
"""
    ...
    desktop_file = Path(join_paths(apps_dir, "jackiebrowser.desktop"))
...
def copy_icon(ctx: BuildContext, icons_dir: Path) -> bool:
    ...
    icon_dest = Path(join_paths(icons_dir, "256x256", "apps", "jackiebrowser.png"))
...
def prepare_appdir(ctx: BuildContext, appdir: Path) -> bool:
    ...
    app_root = join_paths(appdir, "opt", "jackiebrowser")
    ...
    desktop_file = create_desktop_file(
        apps_dir, f"/opt/jackiebrowser/{ctx.NXTSCAPE_APP_NAME}"
    )
...
THIS="$(readlink -f "${{0}}")"
HERE="$(dirname "${{THIS}}")"
export LD_LIBRARY_PATH="${{HERE}}"/opt/jackiebrowser:$LD_LIBRARY_PATH
export CHROME_WRAPPER="${{THIS}}"
"${{HERE}}"/opt/jackiebrowser/{ctx.NXTSCAPE_APP_NAME} "$@"
"""
...
def create_launcher_script(ctx: BuildContext, bin_dir: Path) -> None:
    """Create launcher script in /usr/bin/jackiebrowser."""
    bin_dir.mkdir(parents=True, exist_ok=True)

    launcher_content = f"""#!/bin/sh
# BrowserOS launcher script
export LD_LIBRARY_PATH=/usr/lib/jackiebrowser:$LD_LIBRARY_PATH
exec /usr/lib/jackiebrowser/{ctx.NXTSCAPE_APP_NAME} "$@"
"""

    launcher_path = Path(join_paths(bin_dir, "jackiebrowser"))
...
def create_control_file(ctx: BuildContext, debian_dir: Path) -> None:
    ...
    control_content = f"""Package: jackiebrowser
Version: {version}
Section: web
Priority: optional
...
Maintainer: BrowserOS Team <support@jackiebrowser.com>
Homepage: https://www.jackiebrowser.com/
...
if [ -f /usr/lib/jackiebrowser/chrome_sandbox ]; then
    chmod 4755 /usr/lib/jackiebrowser/chrome_sandbox
fi
...
def prepare_debdir(ctx: BuildContext, debdir: Path) -> bool:
    ...
    ├── usr/
    │   ├── bin/
    │   │   └── jackiebrowser (launcher script)
    │   ├── lib/jackiebrowser/
    │   │   └── [all browser files]
    │   └── share/
    │       ├── applications/jackiebrowser.desktop
    │       └── icons/hicolor/256x256/apps/jackiebrowser.png
    """
    log_info("📁 Preparing .deb directory structure...")

    lib_dir = join_paths(debdir, "usr", "lib", "jackiebrowser")
    ...
    # Create desktop file
    create_desktop_file(apps_dir, "/usr/bin/jackiebrowser")
...
def package_deb(ctx: BuildContext, package_dir: Path) -> Optional[Path]:
    ...
    filename = f"jackiebrowser_{version}_{arch_suffix}.deb"
    output_path = Path(join_paths(package_dir, filename))
